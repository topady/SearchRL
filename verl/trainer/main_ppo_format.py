...
from verl.utils.import_utils import load_extern_function
...
class RewardManager():
    ...
    def __init__(self, tokenizer, num_examine, config=None, structure_format_score=0., final_format_score=0., retrieval_score=0., format_score=0.) -> None:
        ...
        if config is not None:
            custom_cfg = config.get('custom_reward_function', None)
            if custom_cfg is not None:
                self.custom_reward_fn = load_extern_function(custom_cfg.path, custom_cfg.name)
                print(f'[RewardManager] Loaded custom reward function: {custom_cfg.path}:{custom_cfg.name}')
        ...
        for i in range(len(data)):
            ...
            if self.custom_reward_fn is not None:
                score = self.custom_reward_fn(
                    ...
                    extra_info=dict(reward_model_data),
                )
            else:
